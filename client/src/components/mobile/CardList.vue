    ...
    <v-container fluid>
        ...
        <v-tabs fixed-tabs slider-color="black" color="#fafafa" v-model="model">
         ...
         <v-layout justify-center class="mt-3">
            <v-flex xs12>
                <v-text-field label="Search novel..." color="black" append-icon="search" v-model="search" single-line clearable></v-text-field>
            </v-flex>
        </v-layout>
        ...
        <v-layout column justify-center>
            <v-flex xs12 v-for="novel in paginatedList" :key="novel.id">
                <v-hover>
                    <v-card slot-scope="{ hover }" :class="`elevation-${hover ? 3 : 0}`" class="text-xs-center ma-4 mx-5 rounded">
                        <novelCard :novel="novel" @novelUpdated="$emit('novelUpdated');" />
                    </v-card>
                </v-hover>
            </v-flex>
        </v-layout>
        <v-layout justify-center v-if="filterSearch.length > 0">
                <v-pagination
                v-model="pagination.page"
                :length="pagination.length"
                :total-visible="pagination.visible"
...
export default {                    
    ...
    computed: {
        ...
        paginatedList() {
            return !this.search ? this.filteredNovels.slice((this.pagination.page-1)*this.visibleNovels, ((this.pagination.page-1)*this.visibleNovels)+this.visibleNovels) : this.filterSearch.slice((this.pagination.page-1)*this.visibleNovels, ((this.pagination.page-1)*this.visibleNovels)+this.visibleNovels);
        },
        filterSearch() {
            return this.search ? this.filteredNovels.filter(novel => novel.title.toLowerCase().match(this.search.toLowerCase())) : this.filteredNovels
        }
    },
    methods: {
        updatePaginationLength(watcher) {
            ...
            if (!this.search) {                                                                     // HANDLE DEFAULT FILTERED PAGINATION 
                if (this.filteredNovels.length % this.visibleNovels == 0 && this.filteredNovels.length != 0) {
                    paginationLength = this.filteredNovels.length / this.visibleNovels
                } else {
                    paginationLength = Math.floor(this.filteredNovels.length / this.visibleNovels) + 1 
                }
            } else {                                                                               // HANDLE SEARCH-FILTERED PAGINATION
                if (watcher == 'search') {
                    if (this.filterSearch.length % 10 == 0) {
                        paginationLength = this.filterSearch.length / this.visibleNovels;
                    } else {
                        paginationLength = Math.floor(this.filterSearch.length / this.visibleNovels) + 1
                    }
                } else {
